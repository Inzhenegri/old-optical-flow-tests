...
cap = cv.VideoCapture(r"test_videos/test3.avi")
...
#    cv.polylines(allBinary_visual, [src_draw], True, 255)
    ...
    pointleft = midpoint-50
    pointright = midpoint+50
    ...
    cv.line(warped_visual, (pointleft, 0), (pointleft, warped_visual.shape[0]), 110, 2)
    cv.line(warped_visual, (pointright, 0), (pointright, warped_visual.shape[0]), 110, 2)
...
#        
    ...
    if np.sum(leftx, 0) == 0 | np.sum(lefty, 0) == 0:
        ...
        center_fit[2] = right_fit[2]/2
    else:
        if np.sum(rightx, 0) == 0 | np.sum(righty, 0) == 0:
            left_fit = np.polyfit(lefty, leftx, 2)
            center_fit = left_fit
            center_fit[2] = left_fit[2] / 2
        else:
            right_fit = np.polyfit(righty, rightx, 2)
            left_fit = np.polyfit(lefty, leftx, 2)
